...
**参数说明**:
- `算法`: 0=均值法, 1=高斯法（推荐）
- `块大小`: 局部区域大小（必须为奇数，3-99）
- `常数C`: 阈值偏移量（-10到10）

---

### 3. 直方图均衡化 (HistogramEqualizeNode)

**功能**: 增强图像对比度，支持全局和CLAHE两种模式
...
**参数说明**:
- `模式`: 0=全局均衡化, 1=CLAHE（推荐）
- `CLAHE限制`: 对比度限制（1-40，越大效果越强）
- `网格大小`: 分块大小（2-16，越小局部性越强）
