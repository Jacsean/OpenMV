...
class SobelEdgeNode(BaseNode):
    ...
    def __init__(self):
        ...
        self.add_input('输入图像', color=(100, 255, 100))
        self.add_output('输出图像', color=(100, 255, 100))
        self.add_combo_menu('direction', '方向',
                           items=['X方向', 'Y方向', 'XY方向'],
                           tab='properties')
    ...
    def process(self, inputs=None):
        ...
        if inputs and len(inputs) > 0 and inputs[0] is not None:
            ...
            try:
                ...
                dst_uint8 = np.uint8(dst_normalized)
                
                # 创建彩色图像并在角点处标记
                result = image.copy()
                result[dst > threshold * dst.max()] = [0, 0, 255]  # 红色标记角点
                
                return {'输出图像': result}
            except Exception as e:
                print(f"Harris角点检测错误: {e}")
                return {'输出图像': None}
        return {'输出图像': None}
...
class HoughLinesNode(BaseNode):
    ...
    def __init__(self):
        ...
        self.set_property('theta', '0.0174533')  # pi/180
